    ...
    Microsoft.Extensions.Options.IOptions<AiLlmOptions> options) : IAiAdvisorService
{
    ...
    static readonly string SystemPromptTemplate = """
        ...
        Используй ТОЛЬКО факты из JSON-контекста ниже. Не выдумывай цифры и имена.
        Дай 3–7 конкретных, приоритизированных рекомендаций с опорой на цифры из контекста.

        Структура JSON (основные блоки):
        - KPI: totalDeals, conversionPct, monthRevenueUsd, monthRevenueGrowthPct, openPipelineUsd, weightedPipelineUsd, overdueTasks, staleOpenDealsOver30Days, dealsClosingWithin14Days
        - stageSlices / monthlyRevenue / avgDaysPerFunnelStage — воронка и динамика
        - openDeals — открытые сделки (сумма, вероятность, daysInStage, expectedClose)
        - recentClosedDeals — недавние закрытые сделки
        - clientInsights — клиенты (openAmountUsd, daysSinceLastContact)
        - recentClientInteractions — касания с клиентами
        - overdueTaskList / upcomingTasks — задачи
        - recentTeamActivity — лента активности команды
        - recentAuditHighlights — последние изменения в CRM
        - pipelines — воронки продаж
        - team, integrations, support — команда, интеграции, обращения в поддержку
        - dataNotAvailableInCrm — чего нет в системе (не выдумывай)

        Если данных мало — скажи честно и предложи, что начать учитывать в CRM.
